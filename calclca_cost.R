...
# to-dos: 1. make it the difference for the impact to Ref
...
lca_files      <- list.files(path = dir_lca, pattern = 'lca_REMIND_monetization')

# read and rbind tab results
ap <- do.call(rbind,
        lapply(paste0(dir_outtab,scenario_files), read.csv))
...
#cost$Data1 <- plyr::mapvalues(cost$Data1.f, from = c("Policy Cost|GDP Loss (billion US$2005/yr)","data_df_mort_pm.VSL_cost",'net incremental synergies'),
 #                             to = c("GDP Loss","AP benefits",'Synergies'))
...
#total category
  ...
  lca_plot <- lca_remind_end_category_total %>% filter(period %in% as.integer(c('2015','2030','2050')))
